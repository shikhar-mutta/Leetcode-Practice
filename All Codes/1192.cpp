...
// TC: O(n + e), SC: O(n + e)
// Approach: Tarjan's Algorithm for finding bridges in a graph. The code uses an iterative DFS approach with a stack to avoid recursion depth issues. It constructs a compressed sparse row (CSR) representation of the graph and maintains discovery and low-link values to identify critical connections (bridges).
