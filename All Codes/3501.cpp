...
// TC: O(nlogn + qlogn), SC: O(nlogn)
// Approach:
//  1. Count the number of active sections and store the indices of inactive sections in a vector.
//  2. For each query, find the indices of the inactive sections that are within the query range and calculate the maximum number of active sections that can be obtained by trading the inactive sections.
//  3. Use a sparse table to efficiently find the maximum number of inactive sections that can be traded within a given range.
//  4. Return the maximum number of active sections for each query.
