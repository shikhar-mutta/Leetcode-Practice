...
//  Approach: root the tree at 1, compute an Euler tour (in/out time per node) and the base distance from root using the original edge weights. Updating the weight of edge (u,v) (child = the endpoint whose parent is the other) by delta shifts the distance of every node in that child's subtree by delta, which is exactly a Euler-tour range-update handled by a Fenwick tree (range add / point query via prefix sum). Distance queries answer as baseDist[node] + BIT.prefixSum(in[node]).
class Solution
{
public:
    int mod = 1e9 + 7;
