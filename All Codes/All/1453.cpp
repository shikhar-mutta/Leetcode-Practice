...
// TC: O(n^3) SC: O(1)
// Approach: For each pair of darts, we can find the two possible centers of the circle that passes through both darts and has a radius of r. Then, we can count how many darts are inside the circle for each center and keep track of the maximum count.
class Solution
{
private:
    static inline double Sqr(double val) { return val * val; }

    static inline double Sqr_Distance(double x1, double y1, double x2,
                                      double y2)
    {
        ...
        for (size_t i = 0; i != darts.size(); ++i)
        {
            ...
            for (size_t j = i + 1; j != darts.size(); ++j)
            {
                ...
                for (auto &pos : darts)
                {
                    ...
                    y = pos[1];
#define EPS 1e-6
                    if ((Sqr_Distance(xC1, yC1, x, y) - EPS) < Sqr_R)
                        ++count1;
                    if ((Sqr_Distance(xC2, yC2, x, y) - EPS) < Sqr_R)
                        ++count2;
                }
                if (count1 > max)
                    max = count1;
