...
// TC: O(nlogn)  SC: O(n)
// Approach: We can use a greedy approach to solve this problem. We can sort the restrictions based on the index of the buildings. We can then iterate through the restrictions and for each restriction, we can calculate the maximum height of the building that can be built at that index. We can then update the maximum height of the building that can be built at that index based on the previous restriction. We can then iterate through the restrictions in reverse order and update the maximum height of the building that can be built at that index based on the next restriction. Finally, we can iterate through the restrictions and calculate the maximum height of the building that can be built between two restrictions.
