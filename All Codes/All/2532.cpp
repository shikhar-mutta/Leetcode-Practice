...
//  index desc) for workers waiting on each bank, min-heaps (by finish time) for workers busy picking/putting. Right-bank waiters always get bridge priority over left-bank waiters. If the bridge is idle with no one waiting, fast-forward time to the next finishing worker.
class Solution
{
public:
    int findCrossingTime(int n, int k, vector<vector<int>> &time)
    {
        ...
        {
            if (rr.empty() && (r.empty() || r.top().first > free) &&
                (!n || ll.empty() && (l.empty() || l.top().first > free)))
            {
                int cand = INT_MAX;
                if (n && l.size())
                    cand = min(cand, l.top().first);
                if (r.size())
                    cand = min(cand, r.top().first);
                free = cand;
            }
