    ...
    bool maxSubstringLength(string s, int k) {
        ...
        for (int x = 0; x < 26; x++) {
            ...
            for (int y = x + 1; y < 26; y++) {
                ...
                if (lx < fy || ly < fx) continue; // disjoint ranges
                ...
                if (xContainsY && !yContainsX) {
                    ...
                    needUnion = true; // crossing overlap (or identical ranges)
                }
                if (needUnion) {
        ...
        map<int, pair<int,int>> comp; // root -> (start,end)
        for (int c = 0; c < 26; c++) {
        ...
        vector<int> best(m, 0);
        // process in order of decreasing start (so children computed before parents), or just process by reverse of a valid topological order
        vector<int> order(m);
