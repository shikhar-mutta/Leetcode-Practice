...
// SC: O(n1+n2) for the dp arrays used to store the maximum numbers created from each vector.
// Approach:
//   1. We can use a dynamic programming approach to solve this problem.
//   2. We can create a dp array for each vector to store the maximum numbers that can be created from each vector of different lengths.
//   3. We can then merge the maximum numbers created from each vector to create the final maximum number of length k.
//   4. We can use a greedy approach to create the maximum number from each vector by iterating through the vector and keeping track of the maximum number that can be created by dropping the minimum number of elements from the vector.
//   5. We can then merge the maximum numbers created from each vector by comparing the elements of the two vectors and adding the larger element to the final result until we reach the desired length k.
//  6. We can use a helper function to create the maximum number from each vector and another helper function to merge the maximum numbers created from each vector.
class Solution
{
public:
...
#define MAX(a, b) (a > b ? a : b)
    ...
    void getMax(int *num, int &len, int *result, int &t, int &sortedLen)
    {
        ...
        for (int i = 1; i < len; ++i)
        {
            ...
            while (top >= 0 && result[top] < n && (i - top) <= need2drop)
                --top; // i - top means already dropped i - top numbers
            if (i - top > need2drop)
            {
                sortedLen = MAX(1, top);
                while (++top < t)
        ...
        while (i < resSize)
        {
            if (j < len1 && k < len2)
            {
                ...
                else
                {
                    ...
                    while (num[cnt] == tmp && cnt < len && i < resSize)
                        result[i++] = num[cnt++];
                }
            }
            else if (j < len1)
    ...
    vector<int> maxNumber(vector<int> &nums1, vector<int> &nums2, int k)
    {
        ...
        if (len1 == 0 && len2 > 0)
            getMax(&nums2[0], len2, res, k, sortedLen2);
        else if (len1 > 0 && len2 == 0)
            getMax(&nums1[0], len1, res, k, sortedLen2);
        else if (len1 > 0 && len2 > 0)
        {
