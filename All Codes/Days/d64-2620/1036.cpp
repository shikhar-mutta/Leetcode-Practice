    ...
    bool bfsEscape(unordered_set<long long> &blockedSet, pair<int, int> start, pair<int, int> end)
    {
        ...
        while (!q.empty())
        {
            ...
            for (int d = 0; d < 4; d++)
            {
                ...
                if (visited.count(k) || blockedSet.count(k))
                    continue;
                visited.insert(k);
                q.push({nx, ny});
            }
...
public:
    bool isEscapePossible(vector<vector<int>> &blocked, vector<int> &source, vector<int> &target)
    {
        ...
        for (auto &p : blocked)
            blockedSet.insert((long long)p[0] * 1000000 + p[1]);

        pair<int, int> s = {source[0], source[1]};
        pair<int, int> t = {target[0], target[1]};

        if (!bfsEscape(blockedSet, s, t))
            return false;
        if (!bfsEscape(blockedSet, t, s))
            return false;
        return true;
    }
};
