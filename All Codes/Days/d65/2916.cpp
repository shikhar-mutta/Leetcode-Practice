...
public:
    ...
    void rangeAdd(int l, int r, long long val) { // 1-indexed inclusive
        bitAdd(bit1, l, val); bitAdd(bit1, r+1, -val);
        bitAdd(bit2, l, val * (l-1)); bitAdd(bit2, r+1, -val * r);
    }
    long long prefixSum(int i) {
        return bitQuery(bit1, i) * i - bitQuery(bit2, i);
    }
    long long rangeSum(int l, int r) { // 1-indexed inclusive
        if (l > r) return 0;
    ...
    int sumCounts(vector<int>& nums) {
        n = nums.size();
        bit1.assign(n+2, 0); bit2.assign(n+2, 0);
