...
// TC: O(n log n)  SC: O(n)
//  Approach: a subarray is balanced when its count of DISTINCT even values
//  equals its count of DISTINCT odd values. Model each distinct value's
//  first-occurrence-so-far as +1 (odd) / -1 (even) added to a running signed
//  total "curr"; a subarray (j, i] is balanced iff curr at i equals curr at j. Since a value's contribution must move when it repeats (only its most recent position counts as the "active" one), maintain curr via a segment tree over positions [0..n] supporting range-add (shift a value's active window with a suffix add) and "leftmost position with a given value" (using per-node min/max to prune the search). For each i, after updating, binary-search the leftmost j in [0,i] with value == curr and take i-j.
struct Node
{
    short min, max, lazy;
...
public:
    ...
    int findLast0(int i, int l, int r)
    {
        ...
        if (segTree[i].min > 0 || segTree[i].max < 0)
            return -1;
