...
// TC: O(n*26)  SC: O(n*26)
//  Approach: DP with 3 states: 0 = start of new block, 1 = second char of block, 2 = third char of block, 3 = after block.
//  For each state, we keep track of the minimum cost to reach that state at each position, and the cost to continue the current block or start a new block. We also keep track of the character used for the current block to ensure that we can extend it correctly. Finally, we reconstruct the result string based on the DP table.
class Solution
{
public:
