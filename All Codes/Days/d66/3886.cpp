...
public:
    int sortableIntegers(vector<int>& nums) {
        ...
        for (int i = 0; i < n; i++) prefix[i+1] = max(prefix[i], (long long)nums[i]);

        vector<long long> suffix(n + 1, LLONG_MAX);
        for (int i = n - 1; i >= 0; i--) suffix[i] = min(suffix[i+1], (long long)nums[i]);
        ...
        auto check = [&](int k) {
            ...
            for (int i = 0; i < n; i += k) {
                if (prefix[i] > suffix[i]) return false;
                int internalDescents = prefix2[i+k-1] - prefix2[i];
                int wrap = (nums[i+k-1] > nums[i]) ? 1 : 0;
                if (internalDescents + wrap > 1) return false;
            }
            return true;
        };

        long long result = 0;
        for (int k = 1; k <= n; k++) if (check(k)) result += k;
        return (int)result;
    }
};
