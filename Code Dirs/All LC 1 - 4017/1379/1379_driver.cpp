...
int main() {
    ...
    while (t--) {
        ...
        TreeNode* original = buildTree(toks);
        TreeNode* cloned = buildTree(toks);
        int targetVal = _ri();
        TreeNode* target = findByVal(original, targetVal);

        Solution sol;
        auto res = sol.getTargetCopy(original, cloned, target);
        cout << (res ? res->val : -1) << "\n";
    }
    return 0;
}
