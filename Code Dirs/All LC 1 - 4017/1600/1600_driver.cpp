...
int main() {
    ...
    while (t--) {
        ...
        ThroneInheritance* ti = nullptr;
        cout << "[";
        for (int i = 0; i < (int)ops.size(); i++) {
            if (i) cout << ", ";
            auto toks = splitTokens(groups[i]);

            if (ops[i] == "ThroneInheritance") {
                ti = new ThroneInheritance(stripQuotes(toks[0]));
                cout << "null";
            } else if (ops[i] == "birth") {
                ti->birth(stripQuotes(toks[0]), stripQuotes(toks[1]));
                cout << "null";
            } else if (ops[i] == "death") {
                ti->death(stripQuotes(toks[0]));
                cout << "null";
            } else if (ops[i] == "getInheritanceOrder") {
                auto order = ti->getInheritanceOrder();
                cout << "[";
