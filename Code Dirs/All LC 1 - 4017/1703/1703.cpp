...
public:
    int minMoves(vector<int>& nums, int k) {
        ...
        for (int i = 0; i < (int)nums.size(); i++) if (nums[i] == 1) pos.push_back(i);
        int m = pos.size();
        vector<long long> p(m);
        for (int i = 0; i < m; i++) p[i] = pos[i] - i;
