...
// TC: O(nlogn + qlogn)
// SC: O(n)
// Approach: Binary Trie + DFS
//  1. We can use a binary trie to store the prefix xor of the path from the root to each node.
//  2. We can use DFS to traverse the tree and build the binary trie for each node.
//  3. For each query, we can find the kth smallest xor sum by traversing the binary trie and keeping track of the number of elements in the left and right subtrees.
//  4. The answer for each query is the kth smallest xor sum from the root to the node specified in the query.
class BinaryTrie
{
    struct Node
    {
