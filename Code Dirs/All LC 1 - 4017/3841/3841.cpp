...
public:
    vector<bool> palindromePath(int n, vector<vector<int>> &edges, string s, vector<string> &queries)
    {
        ...
        auto fenwUpdate = [&](int i, int val)
        { for (i++; i <= n; i += i & (-i)) fenw[i] ^= val; };
        auto fenwQuery = [&](int i)
        { int r = 0; for (i++; i > 0; i -= i & (-i)) r ^= fenw[i]; return r; };
        auto rangeXor = [&](int l, int r, int val)
        { fenwUpdate(l, val); fenwUpdate(r + 1, val); };

        auto curPrefixXor = [&](int node)
        { return baseXor[node] ^ fenwQuery(in[node]); };
