...
vector<vector<string>> _rvvs() {
    ...
    for (char c : s) {
        if (c == '"') { in = !in; continue; }
        if (!in && c == '[') { dep++; if (dep > 2) cur += c; continue; }
        if (!in && c == ']') {
            dep--;
            if (dep == 1) {
                vector<string> row;
                ...
                string tok;
                for (char rc : cur) {
                    if (rc == '"') { inQ = !inQ; continue; }
                    if (!inQ && rc == ',') { row.push_back(tok); tok = ""; continue; }
                    tok += rc;
                }
...
vector<double> _rvd() {
    ...
    string t;
    while (getline(ss, t, ',')) if (!t.empty()) v.push_back(stod(t));
    return v;
}
