...
vector<string> parseArgList(const string& s) {
    vector<string> v;
    if (s.size() < 2) return v;
...
int main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < n && i < (int)groups.size(); i++) args[i] = parseArgList(groups[i]);
        MapSum* ms = nullptr;
        vector<string> outputs;
        for (int i = 0; i < n; i++) {
            if (ops[i] == "MapSum") { ms = new MapSum(); outputs.push_back("null"); }
            else if (ops[i] == "insert") { ms->insert(args[i][0], stoi(args[i][1])); outputs.push_back("null"); }
            else if (ops[i] == "sum") { outputs.push_back(to_string(ms->sum(args[i][0]))); }
        }
        cout << "[";
