...
TreeNode* _rtree() {
    ...
    while (!q.empty() && i < tok.size()) {
        ...
        if (i < tok.size()) {
            if (tok[i] != "null") { node->left = new TreeNode(stoi(tok[i])); q.push(node->left); }
            i++;
        }
        if (i < tok.size()) {
            if (tok[i] != "null") { node->right = new TreeNode(stoi(tok[i])); q.push(node->right); }
            i++;
        }
    }
    return root;
...
int main() {
    ...
    while (t--) {
        ...
        int k = _ri();
        Solution sol;
        auto res = sol.kthLargestLevelSum(root, k);
        cout << res << "\n";
    }
    return 0;
}
