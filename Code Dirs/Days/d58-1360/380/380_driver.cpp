    ...
    while (t--) {
        ...
        for (int i = 0; i < (int)ops.size(); i++) {
            if (ops[i] == "RandomizedSet") {
                ...
                bool res = obj->insert(a[0]);
                if (res) trueSet.insert(a[0]);
                outputs.push_back(res ? "true" : "false");
            } else if (ops[i] == "remove") {
                vector<int> a = _parseVi(args[i]);
                bool res = obj->remove(a[0]);
                if (res) trueSet.erase(a[0]);
                outputs.push_back(res ? "true" : "false");
            } else if (ops[i] == "getRandom") {
                int res = obj->getRandom();
                outputs.push_back(trueSet.count(res) ? "R" : ("BAD:" + to_string(res)));
            }
        }
        cout << "[";
