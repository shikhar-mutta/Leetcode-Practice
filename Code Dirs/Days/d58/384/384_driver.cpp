...
int main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < (int)ops.size(); i++) {
            if (ops[i] == "Solution") {
                original = _parseVi(args[i]);
                obj = new Solution(original);
            } else if (ops[i] == "reset") {
                auto res = obj->reset();
                if (res != original) ok = false;
            } else if (ops[i] == "shuffle") {
                auto res = obj->shuffle();
                vector<int> sortedRes = res, sortedOrig = original;
                sort(sortedRes.begin(), sortedRes.end());
                sort(sortedOrig.begin(), sortedOrig.end());
                if (sortedRes != sortedOrig) ok = false;
            }
        }
        cout << (ok ? "PASS" : "FAIL") << "\n";
        delete obj;
