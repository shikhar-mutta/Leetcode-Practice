...
int main() {
    ...
    while (t--) {
        ...
        Solution sol;
        auto res = sol.findDuplicateSubtrees(root);
        vector<string> serials;
        for (auto* node : res) serials.push_back(_serializeTree(node));
        sort(serials.begin(), serials.end());
        cout << "[";
        for (size_t i = 0; i < serials.size(); i++) { if (i) cout << ","; cout << serials[i]; }
        cout << "]\n";
    }
    return 0;
}
