    ...
    while (t--) {
        ...
        for (size_t i = 0; i < ops.size(); i++) {
            ...
            if (ops[i] == "UndergroundSystem") {
                ...
                obj->checkIn(stoi(parts[0]), parts[1], stoi(parts[2]));
                outputs.push_back("null");
            } else if (ops[i] == "checkOut") {
                obj->checkOut(stoi(parts[0]), parts[1], stoi(parts[2]));
                outputs.push_back("null");
            } else if (ops[i] == "getAverageTime") {
                double res = obj->getAverageTime(parts[0], parts[1]);
                ostringstream oss;
                oss << fixed << setprecision(5) << res;
