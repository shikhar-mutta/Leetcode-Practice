...
int main() {
    ...
    while (t--) {
        ...
        SmallestInfiniteSet* sis = nullptr;
        vector<string> outputs;
        for (int i = 0; i < (int)ops.size(); i++) {
            if (ops[i] == "SmallestInfiniteSet") {
                sis = new SmallestInfiniteSet();
                outputs.push_back("null");
            } else if (ops[i] == "popSmallest") {
                outputs.push_back(to_string(sis->popSmallest()));
            } else if (ops[i] == "addBack") {
                sis->addBack(stoi(rawArgs[i]));
                outputs.push_back("null");
            }
        }
        cout << "[";
