// Link: https://leetcode.com/problems/minimum-equal-sum-of-two-arrays-after-replacing-zeros/description/

// TC: O(n + m) where n and m are lengths of the two arrays
// SC: O(1) - only using constant extra space
// Approach: Calculate sum and zero count for each array. If both have zeros, answer is
// max(sum1 + count1, sum2 + count2). If only one has zeros, check if the other's sum
// is large enough. If neither has zeros, they must be equal.

#include <bits/stdc++.h>
using namespace std;

class Solution
{
public:
    long long minSum(vector<int> &nums1, vector<int> &nums2)
    {
        ios_base::sync_with_stdio(false);
        cin.tie(0);
        cout.tie(0);
        long long z1 = count(nums1.begin(), nums1.end(), 0),
                  z2 = count(nums2.begin(), nums2.end(), 0);
        long long s1 = accumulate(nums1.begin(), nums1.end(), z1),
                  s2 = accumulate(nums2.begin(), nums2.end(), z2);

        if ((s1 < s2 && !z1) || (s2 < s1 && !z2))
            return -1;
        return max(s1, s2);
    }
};