...
// TC: O(n^2), SC: O(n)
// Approach: dp, combinatorics. For each 1 in the binary representation of n, we can either include it or not in the k-reducible number. We can use dynamic programming to count the number of k-reducible numbers less than n by keeping track of the number of 1s included so far and the number of 1s remaining. We can also use combinatorics to calculate the number of ways to choose the remaining 1s from the remaining bits.
