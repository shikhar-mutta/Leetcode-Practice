...
public:
    int beautifulPartitions(string s, int k, int minLength) {
        int n = s.size();
        auto isPrime = [](char c) { return c=='2'||c=='3'||c=='5'||c=='7'; };
        if (!isPrime(s[0]) || isPrime(s[n-1])) return 0;
        if ((long long)k * minLength > n) return 0;
        ...
        for (int j = 1; j <= k; j++) {
            ...
            for (int i = 0; i <= n; i++) {
                ...
                if (iPrime >= 0 && isPrime(s[iPrime])) running = (running + dp[j-1][iPrime]) % MOD;
                if (i >= 1 && !isPrime(s[i-1])) dp[j][i] = running;
            }
        }
        return (int)dp[k][n];
    }
