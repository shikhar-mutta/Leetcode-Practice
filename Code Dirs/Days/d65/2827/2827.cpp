...
public:
    ...
    long long dp(int pos, int diff, int mod, bool tight, bool started) {
        if (pos == len) return (started && diff == 0 && mod == 0) ? 1 : 0;
        if (!tight && started && memo[pos][diff+len][mod] != -1) return memo[pos][diff+len][mod];
        ...
        if (!tight && started) memo[pos][diff+len][mod] = (int)total;
        return total;
    }
