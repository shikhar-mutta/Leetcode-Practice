...
public:
    ...
    int countKSubsequencesWithMaxBeauty(string s, int k) {
        ...
        for (int i = 0; i < 26; i++) if (freq[i] > 0) vals.push_back(freq[i]);
        ...
        for (int i = 0; i < (int)vals.size(); i++) {
            if (vals[i] > boundary) { product = product * vals[i] % MOD; fullyIncluded++; }
            else if (vals[i] == boundary) numAtBoundary++;
        }
        ...
        for (int i = 0; i < needed; i++) boundaryPow = boundaryPow * boundary % MOD;
