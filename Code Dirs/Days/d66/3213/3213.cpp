...
//TC: O(n * m)  SC: O(n + m)
//  Approach: Use Aho-Corasick algorithm to find all suffixes of the current prefix that are in the dictionary. Then use dynamic programming to find the minimum cost to construct the target string. Let dp[i] be the minimum cost to construct the first i characters of the target string.
struct TrieNode
{
...
static TrieNode preallocated_nodes[(int)5e4];
static TrieNode *preallocated_queue[(int)5e4];
static int dp[(int)5e4 + 1];
