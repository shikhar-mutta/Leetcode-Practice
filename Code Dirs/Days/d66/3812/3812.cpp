...
// TC: O(n).SC: O(n). Greedy, topological sort, bitmasks.
// Approach: We can use a greedy approach to solve this problem. We can start from the leaves of the tree and move towards the root. For each leaf node, we can check if it needs to be flipped or not. If it needs to be flipped, we can flip it and also flip its parent node. We can keep track of the edges that need to be flipped using a bitmask. Finally, we can return the indices of the edges that need to be flipped.
