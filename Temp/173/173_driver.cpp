...
// keeps each op's own bracket-level argument-list intact as a raw string
...
TreeNode* _treeFromTokens(vector<string>& toks) {
    ...
    while (t--) {
        ...
        for (int i = 0; i < (int)ops.size(); i++) {
            if (ops[i] == "BSTIterator") {
                // args[0] is like "[[7,3,15,null,null,9,20]]" — strip both bracket levels
                string inner = args[i].substr(2, args[i].size() - 4);
                auto toks = _tokFromStr("[" + inner + "]");
                TreeNode* root = _treeFromTokens(toks);
                obj = new BSTIterator(root);
                out.push_back("null");
            } else if (ops[i] == "next") {
                out.push_back(to_string(obj->next()));
            } else if (ops[i] == "hasNext") {
                out.push_back(obj->hasNext() ? "true" : "false");
            }
        }
        cout << "[";
